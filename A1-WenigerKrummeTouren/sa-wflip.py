...
class WKT:
    ...
    def __init__(
            ...
            cooldown_fn: Callable[[float, float, int], float] = lambda t, i, it: i / it,
    ...
    def _angle(self, p1: int, p2: int, p3: int) -> float:
        ...
        return 180 - np.rad2deg(np.arccos(np.dot(ba, bc) /
                                          self._distance(p2, p1) * self._distance(p2, p3)))

    def _get_shortest_path_n_cost(self, sequence: Tuple[int]) -> Tuple[Tuple[int, ...], float, bool]:  # sequence length is_legal
        ...
        if len(illegal_turns) == 2:
            return (sequence[illegal_turns[1]:] + sequence[:illegal_turns[0]+1],
                    (length - self._distance(illegal_turns[0], illegal_turns[1])) * (1 + 2 * self.constraint_factor),
                    True)
        ...
        if len(illegal_turns) == 1:
            if (self._distance(illegal_turns[0], (illegal_turns[0] - 1) % len(sequence))
                ...
                return (sequence[((illegal_turns[0] + 1) % len(sequence)):] + sequence[:((illegal_turns[0] + 1)%len(sequence))],
                        length - self._distance(illegal_turns[0], (illegal_turns[0] + 1) % len(sequence)),
                        True)
    ...
    def _mutate(self, sequence: Tuple[int, ...]) -> Tuple[int, ...]:
        ...
        return tuple(sequence[:s1 + 1] + sequence[s2:s1:-1] + sequence[s2+1:])  # TODO
    ...
    def solve(self) -> Tuple[Tuple[Tuple[int, int], ...], float]:
        ...
        for _ in range(self.n_shots):
            ...
            _, cost, is_legal = self._get_shortest_path_n_cost(sequence)
        ...
        while temperature > self.end_temperature:
            for i in range(len(sequences)):
                ...
                _, new_cost, new_is_legal = self._get_shortest_path_n_cost(sequence)
        ...
        shortest_path, shortest_cost, _ = self._get_shortest_path_n_cost(min(filter(lambda x: x[2], sequences), key=lambda x: x[1])[0])
